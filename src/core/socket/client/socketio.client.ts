...
import { UrlUtil } from "@cova/core/common/util/url.util";
...
export class SocketIOClient implements ISocketClient {
  ...
  async connect(): Promise<void> {
    if (!this._info.connected) {
      this._client = io(this._info.originalAddress, {
        ...
        transports: this._options.ioTransport
          ? [this._options.ioTransport]
          : EnumUtil.values(SocketIOTransport),
      });

      this._client.onAny((event, ...args) => {
        const message = args[0] === undefined ? null : args[0];
      ...
      try {
        const connect$ = fromEvent(this._client, "connect").pipe(
          RxJSUtil.timeout(
            ...
            `connection to ${this._info.address} is timed out`
          ),
  ...
  private async emit<TData, TMessage>(
    data: TData,
    event: string,
    ...
    if (!client || !connected) {
      throw new Error(
        `error: client is not connected to ${this._info.address}`
      );
    }
    ...
    const reply$ = from(
      new Promise<SocketSendReply>((resolve) => {
    ...
    ).pipe(timeout(this._options.replyTimeout));
  ...
  private receiveMessage(event: string, message: any): AnyObject {
    ...
    if (parsed.format === MessageFormat.ByteArray) {
      ...
      if (encoding) {
        eventMessage[encoding] = Buffer.from(
          parsed.data as Uint8Array
        ).toString(encoding);
      }
    }
