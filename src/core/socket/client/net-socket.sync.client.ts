...
import { AnyObject, MessageUtil, RxJSUtil } from "@cova/core/common";
import {
  ISocketClient,
  ISocketEventStore,
  SocketEventType,
...
export class NetSocketSyncClient implements ISocketClient {
  ...
  async connect(): Promise<void> {
    ...
    const connect$ = fromEvent(socket, "connect").pipe(
      RxJSUtil.timeout(
        1000,
        `connection to ${this._options.address} is timed out`
      )
    );
  ...
  private async send<TMessage>(
    ...
    return firstValueFrom(
      from(socketSendReplyPromise).pipe(
        switchMap((reply) =>
          merge(
