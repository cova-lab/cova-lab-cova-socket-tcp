...
import { SocketEventType, SocketFactory } from "@cova/core/socket";

(async (): Promise<void> => {
  try {
    const version = "1.0.0";

    const config = CliFactory.createParser(version).parse();
    ...
    const server = HttpFactory.createServer(client, version);

    await CliFactory.createInteractive(config).listen(server, client);
  } catch (error) {
    const message =
