...
import {
  ...
  SendDataMessageBody,
  SendMessageResponse,
} from "@cova/api/http/nest/http.schema";
import { EnumUtil } from "@cova/core/common";
import {
  ISocketClient,
  SocketEvent,
  SocketEventType,
...
export class HttpController {
  ...
  getMessage(@Param("id") id: string): SocketEvent {
    const message = this.client.store.list().find((m) => m.id === Number(id));
  ...
  @ApiQuery({
    name: "type",
    enum: EnumUtil.values(SocketEventType),
    isArray: true,
    required: false,
  })
  ...
  getMessageList(@Query() query: GetMessageListQuery): SocketEvent[] {
    return this.client.store.list({ types: query.type });
  }
  ...
  @ApiBody({
    schema: { oneOf: refs(SendDataMessageBody, SendBytesMessageBody) },
  })
