...
import { CliConfig, ICliParser } from "@cova/api/cli";
import {
  AnyObject,
  BinaryEncoding,
  EnumUtil,
...
export class CliParser implements ICliParser {
  ...
  parse(argv?: readonly string[]): CliConfig {
    ...
    const description = `${chalk.bold("cova")} - a socket client with REST API`;
    ...
    const websocketExamples =
      `Examples: ${EOL}` +
      `  $ cova ws ws://127.0.0.1:8080 ${EOL}` +
      `  $ cova ws ws://127.0.0.1:8080/?token=secret ${EOL}` +
      `  $ cova ws ws://127.0.0.1:8080 --header authorization:"Bearer token"${EOL}` +
      `  $ cova ws ws://127.0.0.1:8080 --binary-encoding utf8 ${EOL}` +
      "  $ cova ws ws://127.0.0.1:8080 --reply-timeout 3000 --no-emoji";

    const socketIOExamples =
      `Examples: ${EOL}` +
      `  $ cova io http://127.0.0.1:8080 ${EOL}` +
      `  $ cova io http://127.0.0.1:8080/?token=secret --transport websocket${EOL}` +
      `  $ cova io http://127.0.0.1:8080 --header authorization:"Bearer token"${EOL}` +
      `  $ cova io http://127.0.0.1:8080 --auth user:cova --auth pass:qwerty1${EOL}` +
      `  $ cova io http://127.0.0.1:8080 --binary-encoding utf8 ${EOL}` +
      "  $ cova io http://127.0.0.1:8080 --reply-timeout 3000 --no-emoji";

    const tcpExamples =
      `Examples: ${EOL}` +
      `  $ cova tcp 127.0.0.1:8080 ${EOL}` +
      `  $ cova tcp 127.0.0.1:8080 --sync ${EOL}` +
      `  $ cova tcp 127.0.0.1:8080 --binary-encoding utf8 ${EOL}` +
      "  $ cova tcp 127.0.0.1:8080 --no-emoji";

    const unixExamples =
      `Examples: ${EOL}` +
      `  $ cova unix ./unix.sock ${EOL}` +
      `  $ cova unix ./unix.sock --sync ${EOL}` +
      `  $ cova unix ./unix.sock --binary-encoding utf8 ${EOL}` +
      "  $ cova unix ./unix.sock --no-emoji";
    ...
    program
      ...
      .option(
        ...
        `Set binary encoding ${binaryEncodingChoices}`
      )
      ...
      .action((address: string, options: OptionValues) => {
        config = {
          ...
          headers: this.parseKeyValueOption(
            "-h, --header <key:value...>",
            options.header
          ),
    ...
    program
      ...
      .action((address: string, options: OptionValues) => {
        config = {
          ...
          emoji: options.emoji,
          replyTimeout: Number(options.replyTimeout),
          ioAuth: this.parseKeyValueOption(
            "-a, --auth <key:value...>",
            options.auth
          ),
    ...
    program
      ...
      .option(
        ...
        "Set reply timeout in ms (sync mode only)",
        "1000"
      )
      .option(
        "--binary-encoding <encoding>",
  ...
  private validateConfig(config: CliConfig): CliConfig {
    ...
    if (
      isNaN(config.apiPort) ||
      config.apiPort >= 65536 ||
      config.apiPort < 0
    ) {
  ...
  private choices(choices: string[]): string {
    const joined = choices.map((choice) => `"${choice}"`).join(",");

    return `(choices: ${joined})`;
  }

  private parseKeyValueOption(
    optionName: string,
    optionValue?: string[]
  ): Optional<AnyObject> {
    if (optionValue) {
