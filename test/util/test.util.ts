...
import { HttpFactory } from "@cova/api/http";
import {
  ISocketClient,
  SocketFactory,
  SocketOptions,
...
export class TestUtil {
  static async prepareWSStand(opts: SocketOptions): Promise<WSTestStand> {
    ...
    try {
      ...
      const client = SocketFactory.createClient({
        ...opts,
        address: this.prepareClientAddress(opts.type, wssPort),
      });
  ...
  static async prepareIOStand(opts: SocketOptions): Promise<IOTestStand> {
    ...
    try {
      ...
      const client = SocketFactory.createClient({
        ...opts,
        address: this.prepareClientAddress(opts.type, iosPort),
      });
      await client.connect();
      closeStand.client = client;
  ...
  static async prepareNetStand(opts: SocketOptions): Promise<NetTestStand> {
    ...
    try {
      const netsPortOrAddr =
        opts.type === SocketType.Tcp
      ...
      const client = SocketFactory.createClient({
        ...opts,
        address: this.prepareClientAddress(opts.type, netsPortOrAddr),
      });
      await client.connect();
      closeStand.client = client;
  ...
  static async closeWSStands(
    stands: Array<Partial<WSTestStand>>
  ): Promise<void> {
  ...
  static async closeIOStands(
    stands: Array<Partial<IOTestStand>>
  ): Promise<void> {
    for (const stand of stands) {
  ...
  static async closeNetStands(
    stands: Array<Partial<NetTestStand>>
  ): Promise<void> {
    for (const stand of stands) {
    ...
    readdirSync(join(__dirname, "../server/unix"))
      ...
      .map((filename) =>
        unlinkSync(join(__dirname, "../server/unix", filename))
      );
  }
  ...
  private static prepareClientAddress(
    type: SocketType,
    portOrAddr: number | string
  ): string {
