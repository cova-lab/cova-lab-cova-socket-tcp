import { CliFactory } from "@cova/api/cli";
...
describe("CLI", () => {
  describe("Parser", () => {
    it("WebSocket: Parse default config", async () => {
      const args = ["ws", "ws://127.0.0.1:8080"];
    ...
    it("WebSocket: Validate args", async () => {
      ...
      jest
        ...
        .mockImplementation((code) => (caughtExitCode = code || 0));
      jest
        .spyOn(global.console, "error")
        .mockImplementation((message) => (caughtError = message));
    ...
    it("SocketIO: Parse default config", async () => {
      const args = ["io", "http://127.0.0.1:8080"];
  ...
  describe("Interactive", () => {
    ...
    it("Print events with emoji", async () => {
      ...
      expect(log).toEqual([
        ...
        "00003 [1970-01-1 00:00:00] 📥 Message received:  { format: 'string', data: 'HI, cova!' }",
